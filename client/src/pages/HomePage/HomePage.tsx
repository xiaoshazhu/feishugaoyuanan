...
export default function HomePage() {
  ...
  useEffect(() => {
    const hash = window.location.hash.replace("#", "");
    if (hash && ['ideas', 'info', 'intent', 'submit', 'leaderboard'].includes(hash)) {
      const timer = setTimeout(() => {
        const mainEl = document.getElementById("main-content");
        if (mainEl) {
          mainEl.scrollIntoView({
      ...
      <header className="hero">
        ...
        <nav className="topbar" aria-label="主导航">
          ...
          <div className="nav-actions" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <button className={`ghost small ${activeView === 'intent' ? 'active' : ''}`} onClick={() => changeView("intent")}>发起人想法</button>
            <button className={`ghost small ${activeView === 'submit' ? 'active' : ''}`} onClick={() => changeView("submit")}>投放想法</button>
            <button className={`ghost small ${activeView === 'leaderboard' ? 'active' : ''}`} onClick={() => changeView("leaderboard")}>积分榜</button>
            {userInfo && (
          ...
          <section id="view-leaderboard" className="view active">
            <div className="section-head" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px', marginBottom: '24px' }}>
              ...
              <div style={{ 
                ...
                <button
                  onClick={() => setShowOnlyTop10(true)}
                  style={{
                    border: 'none',
                    background: showOnlyTop10 ? '#ffffff' : 'transparent',
                    color: showOnlyTop10 ? '#1e293b' : '#64748b',
                    padding: '6px 16px',
                    borderRadius: '16px',
                    fontSize: '0.82rem',
                    fontWeight: 'bold',
                    cursor: 'pointer',
                    boxShadow: showOnlyTop10 ? '0 2px 6px rgba(0,0,0,0.06)' : 'none',
                    transition: 'all 0.2s ease'
                  }}
                >
                  显示前十名
                ...
                <button
                  onClick={() => setShowOnlyTop10(false)}
                  style={{
                    border: 'none',
                    background: !showOnlyTop10 ? '#ffffff' : 'transparent',
                    color: !showOnlyTop10 ? '#1e293b' : '#64748b',
            ...
            <div id="leaderboard" className="leaderboard" style={{ display: 'flex', flexDirection: 'column', gap: '14px', maxWidth: '800px', margin: '0 auto' }}>
              {leaderboardData && leaderboardData.length > 0 ? (
                (() => {
                  const displayData = showOnlyTop10 
                    ? (leaderboardData || []).slice(0, 10) 
                    : (leaderboardData || []);
                  
                  return displayData.map((person, index) => {
                    const rank = index + 1;
